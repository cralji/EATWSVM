function [BestPar,ti] = FindParamOvR_ETWSVM_fixedC(X,t,kern)
...
% USING:
%   BestPar=FindParamOvR_ETWSVM_fixedC(X,t,kern)
% INPUTS:
...
%                     for parameters set selected.  
  ...
  for f=1:np
    ...
    for is =1:Ns
        kern.param = Vs(is);
        %% Training model
        svmStructs = OvR_ETWSVM(Xtrain,tTrain,param,kern,Vs(is));
        %% classification
