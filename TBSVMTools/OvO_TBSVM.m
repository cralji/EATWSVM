function model = OvO_TBSVM(X,t,param,kern,kernparam,c11,c12,c21,c22)
...
% USAGE:
%  model = OvO_TBSVM(X,t,param,kern,c11,c12,c21,c22) 
% INPUTS:
...
%   kernparam(optional): numeric array with kernel parameters
...
for k = 1:nModels
    ...
    XX = [X1;X2];
    structs{k,1} = TBSVMTraining(XX,tt,kern,param);
    %% hyperplane ji
    index1 = string(t)==string(labels(ij(k,2)));
    index2 = string(t)==string(labels(ij(k,1)));
    X1 = X(index1,:);
    X2 = X(index2,:);
    tt = [ones(size(X1,1),1);-1*ones(size(X2,1),1)];
    XX = [X1;X2];
    structs{k,2} = TBSVMTraining(XX,tt,kern,param);
end
